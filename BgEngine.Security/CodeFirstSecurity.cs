...
namespace BgEngine.Security.Services
{
    public static class CodeFirstSecurity
    {
        ...
        public static string CreateAccount(string userName, string password, string email, string firstname, string lastname, string timezone, string culture, bool requireConfirmationToken = false)
        {
            CodeFirstExtendedProvider provider = VerifyProvider();
            return provider.CreateAccount(userName, password, email, firstname, lastname, timezone, culture, requireConfirmationToken);
        }
